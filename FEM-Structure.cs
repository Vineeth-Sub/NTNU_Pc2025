...
namespace NTNU_Pc2025
{
    internal class FEM_Structure
    {
        ...
        public FEM_Structure(List<FEM_Element> elements, List<Point3d> nodes)
        {
            Elements = elements;
            Nodes = nodes;
            int totalDOF = nodes.Count * 2;  // 2 DOF per node (X, Y)
        ...
        private void AssembleGlobalMatrix()
        {
            foreach (var element in Elements)
            {
                ...
                int[] dofMapping = { startIndex, startIndex + 1, endIndex, endIndex + 1 };
